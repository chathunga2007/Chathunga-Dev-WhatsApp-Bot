...
const sms = (danuwa, m) => {
	...
	if (m.message) {
		...
		const isVo = ['viewOnceMessage', 'viewOnceMessageV2', 'viewOnceMessageV2Extension'].includes(m.type)
		m.msg = isVo ? m.message[m.type].message[getContentType(m.message[m.type].message)] : m.message[m.type]
		if (m.msg) {
			if (isVo) {
			...
			if (m.quoted) {
				...
				m.quoted.fromMe = m.quoted.sender ? m.quoted.sender.split('@')[0].includes(danuwa.user.id.split(':')[0]) : false
				const isQuotedVo = ['viewOnceMessage', 'viewOnceMessageV2', 'viewOnceMessageV2Extension'].includes(m.quoted.type)
				m.quoted.msg = isQuotedVo ? m.quoted[m.quoted.type].message[getContentType(m.quoted[m.quoted.type].message)] : m.quoted[m.quoted.type]
				if (isQuotedVo) {
